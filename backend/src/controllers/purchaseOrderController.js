...
updatePurchaseOrderStatus: async (req, res) => {
  try {
    ...
    console.log('Raw order.items type:', typeof order.items);

    // Parse items - handle JSON string or array
    let items = [];
    try {
    ...
    if (status === 'Supplied') {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        // Extract item details - using the correct field names from your data
        const itemName = item.itemName || item.name || 'Unknown Item';
        const quantity = item.quantity || 0;
        const unit = item.unit || 'piece';
        const category = 'Materials'; // Default category
        
        console.log(`Processing item ${i + 1}: ${itemName}, Quantity: ${quantity}`);

        if (quantity <= 0) {
          console.log(`⚠️ Skipping ${itemName} - quantity is ${quantity}`);
          continue;
        }
        ...
        try {
          ...
          const insertResult = await db.run(
            ...
            [
              ...
              unit,
              category,
              quantity,      // quantity_supplied - THIS IS THE KEY FIELD
              0,             // quantity_issued
              0,             // quantity_returned
              quantity,      // balance
              order.order_number,
          ...
          console.log(`✅ Created store transaction for ${itemName}: ${quantity} ${unit} (id: ${insertResult.lastID})`);
          successCount++;
        } catch (insertError) {
      ...
      console.log(`✅ Completed: ${successCount} of ${items.length} items added to store for PO ${order.order_number}`);
    }
