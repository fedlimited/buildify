...
app.get('/api/download-mobile-app', (req, res) => {
  console.log('📱 Mobile app download requested');
  
  // Look for APK file in the uploads folder
  const files = fs.readdirSync(uploadsDir);
  const apkFile = files.find(file => file.endsWith('.apk'));
  
  if (!apkFile) {
    console.error('❌ No APK file found in uploads directory');
    return res.status(404).json({ 
      error: 'APK file not found',
      message: 'The mobile app download is temporarily unavailable. Please contact support.'
    });
  }
  
  const apkPath = path.join(uploadsDir, apkFile);
  const fileName = 'BOCHI-Construction-Suite.apk';
  
  console.log(`✅ Serving APK: ${apkFile}`);
...
// Endpoint to get app version info (for the landing page)
app.get('/api/mobile-app-info', (req, res) => {
  const files = fs.readdirSync(uploadsDir);
  const apkFile = files.find(file => file.endsWith('.apk'));
  const fileStats = apkFile ? fs.statSync(path.join(uploadsDir, apkFile)) : null;
  ...
  res.json({
    ...
    fileSize: fileStats ? `${Math.round(fileStats.size / (1024 * 1024))}MB` : '50MB',
    description: 'BOCHI Construction Suite Mobile App - Manage projects on the go',
    available: !!apkFile
  });
});
...
// Gantt chart routes
app.get('/api/projects/:projectId/gantt', authenticateToken, projectController.getProjectGantt);
app.post('/api/projects/:projectId/gantt', authenticateToken, projectController.saveProjectGantt);
