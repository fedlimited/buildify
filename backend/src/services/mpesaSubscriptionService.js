...
class MpesaSubscriptionService {
  constructor() {
    ...
    this.shortcode = process.env.MPESA_SHORTCODE || '174379';  // ← ADD THIS
    this.callbackUrl = process.env.MPESA_CALLBACK_URL || 'https://buildify-backend-kye8.onrender.com/api/subscription/mpesa-callback';  // ← ADD THIS
  ...
  async initiatePayment(phoneNumber, amount, accountReference, transactionDesc) {
    ...
    const requestBody = {
      ...
      TransactionDesc: (transactionDesc || 'Subscription Payment').substring(0, 36)
    };
