...
export function IncomeModule() {
  ...
  const handleSave = async () => {
    ...
    const status = form.amountReceived >= net ? 'Paid' : form.amountReceived > 0 ? 'Partial' : 'Pending';
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-border text-left">
              <th className="px-4 py-3 text-xs">Project</th>
              <th className="px-4 py-3 text-xs">Cert No</th>
              <th className="px-4 py-3 text-xs">Date</th>
              <th className="px-4 py-3 text-xs text-right">Gross</th>
              <th className="px-4 py-3 text-xs text-right">VAT</th>
              <th className="px-4 py-3 text-xs text-right">Retention</th>
              <th className="px-4 py-3 text-xs text-right">Net</th>
              <th className="px-4 py-3 text-xs text-right">Received</th>
              <th className="px-4 py-3 text-xs text-right">Balance</th>
            ...
            {filtered.map(i => {
              ...
              return (
                <tr key={i.id} className="hover:bg-muted/50">
                  ...
                  <td className="px-4 py-2">{i.certificateNo}</td>
                  <td className="px-4 py-2">{formatDate(i.date)}</td>
                  <td className="px-4 py-2 text-right">{formatCurrency(i.grossAmount)}</td>
                  <td className="px-4 py-2 text-right">{formatCurrency(vat)}</td>
                  <td className="px-4 py-2 text-right">{formatCurrency(ret)}</td>
                  <td className="px-4 py-2 text-right font-medium">{formatCurrency(net)}</td>
                  <td className="px-4 py-2 text-right text-success">{formatCurrency(i.amountReceived)}</td>
                  <td className={`px-4 py-2 text-right ${bal > 0 ? 'text-warning' : 'text-success'}`}>{formatCurrency(bal)}</td>
