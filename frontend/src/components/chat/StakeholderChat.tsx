...
export function StakeholderChat({ projectId, projectName }: StakeholderChatProps) {
  ...
  const askQuestion = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/ai/stakeholder/project/${projectId}/ask`, {
        method: 'POST',
        headers: {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-6 right-6 p-4 bg-gradient-to-r from-emerald-500 to-teal-600 text-white rounded-full shadow-lg hover:from-emerald-600 hover:to-teal-700 transition-all duration-300 z-50 group"
      >
